...
Każda wiadomość e-mail została przypisana do jednej z grup, oznaczonych różnymi kolorami, na podstawie podobieństwa cech. W dużej części wykresu (\ref{fig:anal_skup_kmeans_plot_PCA_k3}) obserwujemy wyraźne oddzielenie grup na wykresie, oznacza to, że duża część wiadomości w tych grupach posiada różne charakterystyki i można je w miarę łatwo rozróżnić na podstawie tych cech. Jednakże grupy zauważalnie nakładają się na siebie przy ich "łączeniach", a to mówi nam, że niektóre cechy wiadomości są podobne między grupami i może być je trudniej rozróżnić i prawidłowo przyporządkować.
\newpage
...
\textbf{Algorytm Clara} jest rozszerzeniem metody PAM stworzonym właśnie na potrzeby analizy danych zawierających dużą liczbę obiektów, zatem możemy go zastosować, ponieważ charakteryzuje się on zredukowanym czasem obliczeniowym. Taki aspekt jest osiągany dzięki odpowiedniemu próbkowaniu danych.
\\
\noindent
...
fviz_cluster(part.CLARA, main = "CLARA", ellipse = TRUE, geom = "point", xlab="x", ylab="y", show.clust.cent = TRUE, pointsize = .5) +
  ...
  scale_fill_manual(values = c("darkslategray2", "deepskyblue", "dodgerblue4")) 
@
Obszary na powyższych wykresach nachodzą na siebie.
...
\\
\\
Rysujemy wykres \ref{fig:anal_skup_Fuzzy_field} za pomocą funkcji \texttt{fviz\_cluster} dla algorytmu \textit{Fuzzy-means}.
...
\item \textbf{Metody aglomeracyjne} początkowo traktują każdy obiekt jako osobne skupienie. W następnych etapach najbliższe sobie skupienia są łączone, aż do momentu utworzenia jednego dużego skupienia zawierającego wszystkie obiekty.
...
spam_variables_hierarchic <- spam_variables_hierarchic[sample(nrow(spam_variables_hierarchic), 100), ]
...
@
\noindent
Początkowo wybieramy $\varepsilon = 9$, następnie zwiększymy tę wartość i sprawdzimy jak zmieni się dokładność przypasowania do klastrów.
...
@
\noindent
Dane \textit{spambase}, na wykresie, przedstawiają się jako jedna "zbita" grupa. Algorytm \textit{DBSCAN} dla dwóch wartości $\varepsilon$ (wykres \ref{fig:anal_skup_dbscan2} i \ref{fig:anal_skup_dbscan4}) stosuje podział na tylko jeden klaster. Wynika to z tego, że zwizualizowany zbiór danych \textit{spambase} ma dużą gęstość punktów na całym zbiorze i metoda \textit{DBSCAN} nie potrafi zadecydować kiedy wyznaczyć następny klaster. Dodatkowo możemy zauważyć, że metoda ta określiła wiele punktów w środku zbioru jako obserwacje odstające (czarne punkty).

\subsection{Ocena jakości analizy skupień}

Przeprowadzimy ocenę jakości analizy skupień za pomocą popularnego wskaźnika wewnętrznego, a mianowicie indeksu \textit{silhouette} dla każdego omówionego wcześniej algorytmu. Zanim jednak to zrobimy, opiszemy czym właściwie jest wspomniana metoda.
\\
\\
...
Powyższe wyniki świadczą o tym, że najwyższe średnie wartości wskaźnika \textit{silhouette} osiągane są przy stosowaniu metody \textbf{DBSCAN}, a najgorsze dla metody \textbf{AGNES}.
\\
\\
Zastosujemy również inny wskaźnik wewnętrzny, a mianowicie indeks Dunn'a. Definiujemy go jako
$$
D(C) = \frac{\min_{1 \leq i < j \leq K} \operatorname{dist}(C_i, C_j)}{\max_{1 \leq i \leq K} \operatorname{diam}(C_i)},
$$

gdzie
$$
\begin{aligned}
\operatorname{dist}(C_i, C_j) &= \min_{x_l \in C_i, x_m \in C_j}d(x_l, x_m), \\
\operatorname{diam}(C_i) &= \max_{x_l, x_m \in C_i}d(x_l, x_m) \quad \text{(średnica klastra} \, C_i \text{)}.
\end{aligned}
$$
Zakres wartości $D(C)$ jest następujący $D(C) \in [0,\infty]$, a interpretacja taka sama jak w przypadku wskaźnika \textit{silhouette}, im wyższa wartość $D(C)$ tym lepiej. 

Możemy użyć funkcji \texttt{dunn} z pakietu \texttt{clValid}, aby obliczyć wskaźnik Dunna dla danej partycji klastrowej.
\item \textbf{K-means}
<<ocena_dunn_kmeans, echo=F, eval=T, warning=F>>=
dunn_score_kmeans <- dunn(dist(spam_variables), kmeans.k3$cluster)
paste("Dla k = 3",dunn_score_kmeans)

dunn_score_kmeans4 <- dunn(dist(spam_variables), kmeans.k4$cluster)
paste("Dla k = 4",dunn_score_kmeans4)

dunn_score_kmeans6 <- dunn(dist(spam_variables), kmeans.k6$cluster)
paste("Dla k = 6", dunn_score_kmeans6)
@

\item \textbf{CLARA}

<<ocena_dunn_clara, echo=F, eval=T, warning=F>>=
dunn_score_CLARA <- dunn(dist(spam_variables), part.CLARA$cluster)
paste(dunn_score_CLARA)
@

\item \textbf{Fuzzy c-means}

<<ocena_dunn_fuzzy, echo=F, eval=T, warning=F>>=
dunn_score_fuzzy <- dunn(dist(spam_variables), partycja.fcm$cluster)
paste(dunn_score_fuzzy)
@

\item \textbf{AGNES}

<<ocena_dunn_AGNES_assign, echo=F, eval=T, warning=F>>=
clust_assign_s <- cutree(agnes.single, k = 3)
clust_assign_c <- cutree(agnes.complete, k = 3)
clust_assign_a <- cutree(agnes.average, k = 3)
@

<<ocena_dunn_AGNES, echo=F, eval=T, warning=F>>=
dunn_score_agnes_s <- dunn(dist(spam_variables_hierarchic), clust_assign_s)
paste("Dla single linkage:", dunn_score_agnes_s)

dunn_score_agnes_c <- dunn(dist(spam_variables_hierarchic), clust_assign_c)
paste("Dla complete linkage:", dunn_score_agnes_c)

dunn_score_agnes_a <- dunn(dist(spam_variables_hierarchic), clust_assign_a)
paste("Dla average linkage:", dunn_score_agnes_a)

@

\item \textbf{DBSCAN}

<<ocena_dunn_dbscan, echo=F, eval=T, warning=T>>=
dunn2 <- function(distance = NULL, clusters, Data = NULL, method = "euclidean") 
{
    ...
    dunn <- 8/max(intraClust)
    return(dunn)
}

dunn_score_dbscan1 <- dunn2(dist(spam_variables), spam.dbscan1$cluster)
paste("Dla eps = 9:",dunn_score_dbscan1)

dunn_score_dbscan2 <- dunn(dist(spam_variables), spam.dbscan2$cluster)
paste("Dla eps = 7:",dunn_score_dbscan2)

dunn_score_dbscan3 <- dunn2(dist(spam_variables), spam.dbscan3$cluster)
paste("Dla  eps = 11:", dunn_score_dbscan3)

@
Ponownie jak w przypadku metody \textit{silhouette} najlepsze wyniki osiągane są przy stosowaniu metody \textbf{DBSCAN}, a najgorsze dla metody \textbf{CLARA}. Oznacza to, że metoda \textit{DBSCAN} najlepiej przyporządkowuje obiekty oraz ma najwyższą jakość skupienia (oczywiście biorąc pod uwagę tylko te metody, które pokazaliśmy).
\end{itemize}

\section{Redukcja wymiaru}
Celem redukcji wymiaru mogą być takie aspekty jak: eliminacja zbędnej lub nadmiarowej informacji, znalezienie popdprzestrzeni o mniejszym wymiarze, w których koncentruje się większość obserwacji, ekstrakcja cech, wizualizacja oraz identyfikacja struktury zależności w danych. Podejściem, którym się zajmiemy będzie \textit{feature extraction}, gdzie konstruujemy nowe cechy na podstawie oryginalnych, a wtedy możemy odwzorować dane do nowej przestrzeni. Nowe cechy są odpowiednimi kombinacjami liniowymi oryginalnych. Przykładem takiego działania, który zastosujemy będzie \textbf{metoda PCA (Principal Component Analysis)}. Ta metoda redukcji wymiaru jest przykładem uczenia nienadzorowanego, w którym to nie wykorzystujemy informacji nt. przynależności obiektów do klas, a jego celem jest wykrycie wewnętrznej struktury danych.
\\
\noindent
...
@
\noindent
Przydatnym narzędziem graficznym wykorzystywanym do znalezienia optymalnej liczby składowych jest wykres osypiskowy (ang. scree plot). Ustalamy frakcję wyjaśnionej wariancji na $80\% $ i wybieramy tę wartość dla której wykres osypiskowy (rysunek \ref{fig:redukcja_wymiaru_pca_var_sum}) po raz pierwszy zbliżony jest do linii poziomej.
...
<<redukcja_wymiaru_pca4, echo=F, eval=T, fig.width=8, fig.height=2,fig.pos="H", fig.align="center", out.extra="", fig.cap="Tabela korelacji zmiennych i składowych głównych",warning=FALSE>>=
...
Zatem na podstawie powyższego wykresu możemy zapisać na przykład, że zmienne \textit{hpl}, \textit{hp} i \textit{lab} są dodatnio skorelowane, podobnie jak \textit{capitalLong},\textit{capitalTotal} i \textit{capitalAve}. Natomiast zmienne \textit{edu} i \textit{order} są ujemnie skorelowane, a zmienne \textit{capitalLong} i \textit{you} są nieskorelowane.
\end{itemize}

\newpage
Przedstawimy wkład zmiennych do dwóch składowych głównych PC1 i PC2.
...
<<redukcja_wymiaru_pca8, echo=F, eval=T, fig.width=6, fig.height=3.5,fig.pos="H", fig.align="center", out.extra="", fig.cap="Wkład zmiennych do PC2",warning=FALSE>>=
fviz_contrib(pca.results, choice="var", axes=2)
@
Przerywana linia wykresów \ref{fig:redukcja_wymiaru_pca7} i \ref{fig:redukcja_wymiaru_pca8} oznacza oczekiwany średni wkład równy $\frac{1}{p}$ (czyli $\frac{1}{51}$).
...
@
Przedstawimy wykresy dla przypadków i dwuwykresy.
<<redukcja_wymiaru_pca10, echo=F, eval=T, fig.width=5.5, fig.height=3.5,fig.pos="H", fig.align="center", out.extra="", fig.cap="Wykres rozrzutu w przestrzeni (PC1,PC2)",warning=F>>=
fviz_pca_ind(pca.results, repel=TRUE)
@

<<redukcja_wymiaru_pca12, echo=F, eval=T, fig.width=5.5, fig.height=3.5,fig.pos="H", fig.align="center", out.extra="", fig.cap="Dwuwykres z usuniętymi nazwami przypadków",warning=F>>=
fviz_pca_biplot(pca.results, label="var")
@
\noindent
Na koniec zajmiemy się przedstawieniem dwóch najlepszych metod (według wskaźnika \textit{silhouette}), czyli \textit{kmeans} oraz \textit{DBSCAN}, z danymi zredukowanymi metodą PCA i sprawdzimy, czy dokładność dopasowania poprawiła się.

<<redukcja_wymiaru_pca_kmeans, echo=F, eval=T, fig.width=5.5, fig.height=3.5,fig.pos="H", fig.align="center", out.extra="", fig.cap="Zastosowanie nowej reprezentacji danych do analizy skupień (algorytm k-means k = 3)",warning=F>>=

dane2d=pca.results$ind$coord[,1:2]
kmeans3 = kmeans(dane2d, nstart=10, centers = 3)
fviz_cluster(kmeans3, dane2d)

kmeans4 = kmeans(dane2d, nstart=10, centers = 4)
kmeans6 = kmeans(dane2d, nstart=10, centers = 6)
@

<<ocena_silhouette_kmeans_redukcja, echo=F, eval=T>>=
silhouette_score_kmeans_redukcja <- silhouette(kmeans3$cluster, dist(spam_variables))
paste("Dla k = 3",summary(silhouette_score_kmeans_redukcja)$avg.width)

silhouette_score_kmeans4_redukcja <- silhouette(kmeans4$cluster, dist(spam_variables))
paste("Dla k = 4",summary(silhouette_score_kmeans4_redukcja)$avg.width)

silhouette_score_kmeans6_redukcja <- silhouette(kmeans6$cluster, dist(spam_variables))
paste("Dla k = 6", summary(silhouette_score_kmeans6_redukcja)$avg.width)
@

<<redukcja_wymiaru_pca_dbscan, echo=F, eval=T, fig.width=5.5, fig.height=3.5,fig.pos="H", fig.align="center", out.extra="", fig.cap="Wizualizacja dla metody DBSCAN epsilon = 11 dla danych zredukowanych",warning=F>>=
spam.dbscan_redukcja1 <- dbscan::dbscan(dane2d, 9, 5)
spam.dbscan_redukcja2<- dbscan::dbscan(dane2d, 7, 5)
spam.dbscan_redukcja3 <- dbscan::dbscan(dane2d, 11, 5)

fviz_cluster(spam.dbscan_redukcja3, spam_variables, geom = "point")
@

<<ocena_silhouette_dbscan_redukcja, echo=F, eval=T>>=
silhouette_score_dbscan1_redukcja <- silhouette(spam.dbscan_redukcja1$cluster, dist(spam_variables))
paste("Dla eps = 9:",summary(silhouette_score_dbscan1_redukcja)$avg.width)

silhouette_score_dbscan2_redukcja <- silhouette(spam.dbscan_redukcja2$cluster, dist(spam_variables))
paste("Dla eps = 7:",summary(silhouette_score_dbscan2_redukcja)$avg.width)

silhouette_score_dbscan3_redukcja <- silhouette(spam.dbscan_redukcja3$cluster, dist(spam_variables))
paste("Dla  eps = 11:", summary(silhouette_score_dbscan3_redukcja)$avg.width)
@
\noindent
Po zastosowaniu redukcji wymiarów zauważamy znaczną poprawę jakości dla metody \textit{DBSCAN}. Najbardziej zauważalną poprawą będzie brak czarnych punktów w środku klastra oznaczających obserwacje odstające (rysunek \ref{fig:redukcja_wymiaru_pca_dbscan}). Dodatkowo następuje lekka poprawa dla metody \textit{kmeans} dla k = 3 (wykres \ref{fig:redukcja_wymiaru_pca_kmeans}). Wnioskujemy zatem, że redukcja wymiarów znacznie poprawia jakość analizy skupień.
